...
function Get-DotNetVersion {
    ...
    if ($targetFramework -match "net48") {
        return "Framework 4.8"
    }
...
# アップロードするファイルを選択
$packages = Get-ChildItem -File | Where-Object Name -match "^NeeRuler(\d+\.\d+)(-Canary\d+|-Beta\d+)?\.zip$"
...
if ($package.Name -match "^[^\d]+(.+)\.zip$") {
    $release_name = "NeeRuler " + $Matches[1]
...
# .NET version
$dotNetVersion = Get-DotNetVersion  ..\NeeRuler\NeeRuler.csproj


# リリースを作成

$product_body = @"
This is the stable version of NeeRuler $version.
...
Changelog: [Version $version](https://neelabo.github.io/NeeRuler/changelog.html#$version_id)
"@

$canary_body = @"
## Canary Version

NeeRuler Canary is a snapshot of the development process.
It is intended to give you a preview of features that will be available in a future stable version.

This version runs on .NET $dotNetVersion.

See also: [About Canary Version](https://neelabo.github.io/NeeRuler/package-canary.html)
"@

$beta_body = @"
## Beta Version

NeeRuler Beta is the version just before the stable version.
No new features will be added, only bug fixes. The stable version will be released in approximately one to two weeks.

This version runs on .NET $dotNetVersion.

See also: [About Beta Version](https://neelabo.github.io/NeeRuler/package-beta.html)
